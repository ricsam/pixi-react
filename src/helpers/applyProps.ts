...
import { isNull, isUndefined } from "./compare";
  ...
  while (changeIndex < changes.length) {
    ...
    if ((key as string) === "draw" && typeof value === "function") {
      if (instance instanceof Graphics) {
        ...
        log(
          "warn",
          `The \`draw\` prop was used on a \`${instanceState.type}\` component, but it's only valid on \`graphics\` components.`,
        );
      }
    }
    ...
    if (key in PixiToReactEventPropNames) {
      ...
      if (!PIXI_EVENT_PROP_NAME_ERROR_HAS_BEEN_SHOWN[key]) {
        ...
        log(
          "warn",
          `Event names must be pascal case; instead of \`${key}\`, you probably want \`${PixiToReactEventPropNames[typedKey]}\`.`,
        );
      }
    }
    ...
    if (!hasError) {
      ...
      if (keys.length) {
        ...
        if (
          !(
            targetProp && (targetProp as unknown as Record<string, unknown>).set
          )
        ) {
          ...
          currentInstance = reverseEntries
            .reverse()
            .reduce(targetKeyReducer, currentInstance);
      ...
      if (isEvent && instanceState) {
        ...
        if (value) {
          currentInstance[pixiKey] = value as (
        ...
        !isReadOnlyProperty(currentInstance as Record<string, unknown>, key)
      ) {
