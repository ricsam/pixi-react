...
export function insertBefore(
  ...
  invariant(
    childInstance !== beforeChildInstance,
    "Cannot insert node before itself",
  );

  if (childInstance instanceof Container) {
    const childContainerInstance =
      childInstance as HostConfig["containerInstance"];
    ...
    const targetIndex = instanceState.filters.indexOf(
      beforeChildInstance as unknown as Filter,
    );
