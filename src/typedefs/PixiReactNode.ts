...
export interface BaseNodeProps<
  T extends new (...args: any) => any = typeof Container,
> {
...
export type PixiReactNode<
  T extends new (...args: any) => any = typeof Container,
> = NodeProps<InstanceType<T>> &
  EventHandlers & {
    [K in keyof InstanceType<T> as K]: K extends keyof NodeProps<
      InstanceType<T>
    >
...
> = BaseNodeProps<InstanceType<T>> &
  ...
  OmitKeys<
    ExcludeFunctionProps<ConstructorOptions<T>>,
    NodeProps<T> & typeof PixiToReactEventPropNames
  > &
  EventHandlers;
