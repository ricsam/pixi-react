...
describe("useAssets", async () => {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        loaded = {
          ...loaded,
          ...assets.reduce(
            (acc, val) => ({ ...acc, [val.src!.toString()]: Texture.EMPTY }),
            {},
          ),
        };
        data = {
          ...data,
          ...assets.reduce(
            (acc, val) => ({ ...acc, [val.src!.toString()]: val.data }),
            {},
          ),
        };
        resolve(loaded);
  ...
  get.mockImplementation((keys) =>
    keys.reduce<Record<string, unknown>>(
      (acc, key, idx) => ({ ...acc, [idx]: loaded[key] }),
      {},
    ),
  );
  ...
  it("loads assets", async () => {
    ...
    expect(result.current.assets).toEqual(
      assets.map(({ src }) => defaultResults[src!.toString()]),
    );
  });
  ...
  it("accepts data", async () => {
    ...
    const isTexture = (texture?: Texture) =>
      texture && texture instanceof Texture;
  ...
  it("is properly typed with data", async () => {
    ...
    const { result } = renderHook(() =>
      useAssets([{ src: "test.png", data: { test: "d460dbdd" } }]),
    );
